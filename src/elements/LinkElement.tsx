...
function LinkElement(props: { key: string, id: string, nome: string, link: string, onDelete: any[], onCopy: any }) {
    ...
    return (
        <li className="space-y-2 lg:space-y-0 lg:space-x-3 flex flex-col lg:flex-row ">
            <input type="text" onClick={copyLink} placeholder="nome" defaultValue={props.nome} readOnly={true} className="lg:w-48 p-2 rounded-lg outline-none border bg-neutral-900 border-neutral-900 hover:bg-neutral-900/50 transition-colors ease-in duration-100 hover:cursor-pointer active:bg-neutral-900" />
